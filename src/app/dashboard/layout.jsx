...
export default async function Layout({ children }) {
    ...
    return (
        <section className="flex flex-row gap-5">
            <nav className=" shadow rounded w-fit p-10 lg:flex flex-col items-start gap-5 hidden h-fit">
            ...
            <main className="w-full">
                <div className='flex items-center justify-between mb-10'>
                    <Suspense fallback={<Skeleton className="h-[125px]  w-full rounded-xl" />}>
                        <h1 className='lg:text-2xl font-semibold'>Selamat Datang, <NameAnimation name={user.name} /></h1>
                        <ProfileDashboard />
                    </Suspense>
                </div>
                <Suspense fallback={<Skeleton className="h-[125px] w-full rounded-xl" />}>
