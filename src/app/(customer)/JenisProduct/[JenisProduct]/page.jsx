...
export default async function page({ params }) {
    ...
    try {
        ...
        // Filter products based on JenisProduct
        const filteredProducts = products.filter(product =>
            product.JenisProduct.name.replace(/\s+/g, '-').toLowerCase() === JenisProduct.toLowerCase()
        );

        if (filteredProducts.length === 0) {
            // Redirect if no products found for the given JenisProduct
            redirect('/');
        }

        // Determine header and component based on JenisProduct
        ...
        return (
            <div>Error loading page content. Please try again later.</div>
        );
    }
}
