...
async function CardSoldJualbeliToko({ user }) {
    let response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL_API}/products?toko=${user.id}&tipe=Jual Beli&isAvailable=no`, {
        method: 'GET',
        headers: {
    ...
    if (dataProducts.length === 0) {
        return (
            <div className='flex flex-col items-center justify-center my-10'>
                ...
                />
                <p className=' text-lg'><span className='font-semibold'>@{user.username} </span>Belum Ada Product Terjual</p>
            </div>
        )
    }

    return (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-5 opacity-50'>
            {
...
export default CardSoldJualbeliToko
