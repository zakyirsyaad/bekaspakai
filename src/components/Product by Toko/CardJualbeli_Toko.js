...
function CardJualbeli_Toko({ user }) {
    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL_API}/products?toko=${user.id}&tipe=Jual Beli`, {
                    method: 'GET',
                    headers: {
    ...
    if (dataProducts.length === 0) {
        return (
            <div className='self-center'>
                ...
                />
                <p className='text-lg'><span className='font-semibold'>@{user.username} </span>Belum Memiliki Produk Jual Beli</p>
            </div>
        );
    }
